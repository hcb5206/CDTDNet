...
def data_loader(seq_len, targets_len):
    ...
    PM25_s = pd.DataFrame({'Time': t, 'traffic': PM25})

    lag_order = 2
    for i in range(1, lag_order + 1):
        PM25_s[f'Value_Lag_{i}'] = PM25_s['traffic'].shift(i)

    PM25_s['Value_Diff'] = PM25_s['traffic'].diff()
    ...
    train_out, eval_out, test_out, train_targets, eval_targets, test_targets, targets_min, targets_max = normalize_data(
        train_sequences_wavelet, eval_sequences_wavelet, test_sequences_wavelet, train_targets_s, eval_targets_s,
        test_targets_s)
