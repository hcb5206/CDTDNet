...
def add_gaussian_noise(data, noise_std):
    gaussian_noise = noise_std * np.random.randn(len(data))
    data_with_gaussian_noise = data + gaussian_noise
    return data_with_gaussian_noise


def normalize_data(data, data_all, noise_std):
    data_train = []
    for i in range(len(data)):
        min_val = np.min(data[i])
        max_val = np.max(data[i])
        data_a = data_all[i]
        data_g = add_gaussian_noise(data_a, noise_std)
        data_s = (data_g - min_val) / (max_val - min_val)
        data_train.append(data_s)
        if i == 0:
            data_target = (data_a - min_val) / (max_val - min_val)
            targets_min = min_val
            targets_max = max_val
...
def data_loader(seq_len, targets_len, noise_std):
    ...
    air_normalize, PM25_normalize, targets_min, targets_max = normalize_data(num_data, air_imformation, noise_std)
