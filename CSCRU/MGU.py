...
class Lstmcell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.wf = nn.Parameter(torch.Tensor(self.concat_len, self.hidden_size))
    ...
    def forward(self, x, h_prev=None):
        ...
        self.g = torch.tanh(torch.matmul(self.f * self.h_prev, self.whh) + torch.matmul(self.xl, self.whx) + self.bh)
        # self.o = torch.sigmoid(torch.matmul(self.xc, self.wo) + self.bo)
        # self.s = self.g * self.i + self.s_prev * self.f
        self.h = (1 - self.f) * self.h_prev + self.f * self.g
        return self.h
